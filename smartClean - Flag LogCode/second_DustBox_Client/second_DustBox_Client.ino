...
LiquidCrystal_I2C lcd(0x27, 16, 2); // LCD 주소(0x27 또는 0x3F)
...
float threshold = 0.3f;             // 채터링 완화 (좀더 3.0f)
    ...
    while (1)
    {
        ...
        if (currentStatus != lastStatus)
        {
            ...
            if (currentStatus == "OPEN")
            {
                ...
                while (millis() - start < 8000)  // 문 열린 후 8초간 로드셀 감시
                {
                    checkLoadcell();
                    delay(200);
                }
...
void sendToServer(float amount)
{
    ...
    {
        recv_buf[len] = '\0';
        float totalGram = atof(recv_buf);

        Serial.printf("수신 총량: %.2f g\n", totalGram);
        
        printLCD(totalGram, amount);
    }
...
// LCD 출력 함수
void printLCD(float totalGram, float inputGram)
{
    lcd.clear();
    ...
    // 호수 + 무게 정보
    lcd.setCursor(0, 1);
    lcd.print(totalGram, 2);
    lcd.print("g");
    lcd.print("/");
    lcd.print(inputGram, 2);
    lcd.print("g");
}
